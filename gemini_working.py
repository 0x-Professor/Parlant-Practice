...
class SimpleGeminiGenerator(p.SchematicGenerator[T], Generic[T]):
    ...
    async def generate(
        ...
        try:
            # Create a simple example for the schema
            dummy_instance = {}
            try:
                # Try to create an empty instance to get required fields
                dummy_instance = schema_type.model_validate({})
            except Exception:
                # If that fails, we need to look at the schema
                ...
                for field_name in required_fields:
                    ...
                    if field_type == "string":
                        dummy_instance[field_name] = "generated_response"
